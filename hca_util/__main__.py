...
def main():
    ...
    parser_config = subparsers.add_parser('config', help='configure AWS credentials')
    ...
    parser_create = subparsers.add_parser('create', help='create an upload directory (authorised user only)')
    parser_create.add_argument('-n', metavar='name', help='optional name for new directory', type=valid_project_name)
    parser_create.add_argument('-p', choices=['u', 'ud', 'ux', 'udx'], default='ux', help='allowed actions ('
                                                                                          'permissions) on new '
                                                                                          'directory. u for upload, '
                                                                                          'x for delete and d for '
                                                                                          'download. Default is ux')

    parser_select = subparsers.add_parser('select', help='select active directory')
    parser_select.add_argument('DIR', help='directory uuid', type=valid_dir)

    parser_list = subparsers.add_parser('list', help='list contents of selected directory')
    parser_list.add_argument('-b', action='store_true', help='list all directories in bucket (authorised user only)')

    parser_dir = subparsers.add_parser('dir', help='display active (selected) directory')

    parser_upload = subparsers.add_parser('upload', help='upload files to selected directory')
    ...
    parser_download = subparsers.add_parser('download', help='download files from selected directory')
    ...
    parser_delete = subparsers.add_parser('delete', help='delete files from selected directory')
    group_delete = parser_delete.add_mutually_exclusive_group(required=True)

    group_delete.add_argument('-a', action='store_true', help='delete all files leaving directory empty')
